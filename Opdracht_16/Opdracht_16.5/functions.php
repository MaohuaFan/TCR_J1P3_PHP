...
function PrintTableOptie($result){
    $value = PrintTableTotaleStemmen($result);
    echo "<tr>";
        echo "<td> Aantal uitgebrachte stemmen: " . $value;"</td>";
    echo "</tr>";
    foreach($result as &$data) {
        echo "<tr>";
            echo "<td>". $data['optie'] ."</td>";
            echo "<td>". $data['stemmen'] ."</td>";      
                $percentile = $data['stemmen'] / $value * 100;
            echo "<td>" .round($percentile) . "%";"</td>";
        }
}
function stemmen(){
    $conn = ConnectDb();
...
function OvzTotaleStemmen(){
    $result = GetData("optie");
    PrintTableTotaleStemmen($result);
}

function PrintTableTotaleStemmen($result){
    $value = 0;
    foreach($result as &$data) {
        $value = $value + $data['stemmen'];
    }
    /*
    echo "<tr>";
        echo "<td> Aantal uitgebrachte stemmen: " . $value;"</td>";
...
function OvzPercentageStemmen(){
    $result = GetData("optie");
    PrintTablePercentageStemmen($result);
}

function PrintTablePercentageStemmen($result){
    $value = PrintTableTotaleStemmen($result);
    foreach($result as &$data) {
        $percentile = $data['stemmen'] / $value * 100;
        echo "<tr>";
